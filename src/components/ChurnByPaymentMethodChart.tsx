...
const ChurnByPaymentMethodChart: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Payment method names might be long, so we'll abbreviate them
        const paymentData = (response.data.byPaymentMethod || []).map((item: any) => ({
          ...item,
          displayName: abbreviatePaymentMethod(item.name),
        }));
        setData(paymentData);
      } catch (err: any) {
  ...
  const abbreviatePaymentMethod = (method: string): string => {
    const abbr: { [key: string]: string } = {
      'Electronic check': 'E-Check',
      'Mailed check': 'Mail Check',
      'Bank transfer': 'Bank Trans.',
      'Credit card': 'Credit Card',
    };
    return abbr[method] || method;
  };
  ...
  return (
    <div className="card">
      <h2 className="mb-4 text-lg font-semibold text-gray-900">Churn Rate by Payment Method</h2>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 text-center md:grid-cols-4">
        {data.map((item: any) => (
          <div key={item.name} className="text-sm">
            ...
            <p className="text-xs text-gray-600">{item.churned}/{item.total}</p>
          </div>
        ))}
      </div>
    </div>
...
export default ChurnByPaymentMethodChart;
